...
std::string GetMediaTypeDescription(IMFMediaType* pMediaType)
{
    ...
    for (UINT32 i = 0; i < cAttrCount; i++)
    {
        ...
        else
        {
            ...
            char str[128];
            size_t a;
            wcstombs_s(&a,str, sizeof(str), guidStr, sizeof(guidStr));
        ...
        case MF_ATTRIBUTE_STRING:
        {
            ...
            char str[200];
            size_t a;
            wcstombs_s(&a, str, sizeof(str), TempBuf, sizeof(TempBuf));
            description += std::string(str);
...
bool InternalData::readOutputMediaFormat() {
    assert(pSourceReader);
    HRESULT hr;

    IMFMediaType* pNativeType = nullptr;
    CHECK_HR(pSourceReader->GetNativeMediaType((DWORD)0xFFFFFFFC, 0xFFFFFFFF, &pNativeType),
        "Error retrieving GetNativeMediaType.");

    //SAFE_RELEASE(pFirstOutputType);
    CHECK_HR(pSourceReader->GetCurrentMediaType((DWORD)0xFFFFFFFC, &pFirstOutputType), "Error reading GetCurrentMediaType");
...
bool InternalData::open(std::string filename) {
    finished = false;
    fileName_ = filename;
...
void InternalData::update(float elapsed) {
    ...
    if (pVideoSample) {
        ...
        if (target_texture=="") {
            float texture_height = (float)height * 2;
            target_texture = Texture::GetInstance()->loadTexture((float)width, texture_height, DXGI_FORMAT_R8_UNORM, true, fileName_,"test");
        }
...
bool VideoTexture::create(std::string filename) {
    assert(!internal_data);
    internal_data = std::make_unique<InternalData>();
...
std::string VideoTexture::getTextureName() {
    assert(internal_data);
