...
void DrawOBJ::matViewUpdata(Vector3 eye, Vector3 target, Vector3 up)
{
	eye_ = eye;
	target_ = target;
	up_ = up;
...
void DrawOBJ::SetScale(Vector3 scale)
{
...
Matrix4x4 DrawOBJ::matScaleGeneration(Vector3 scale)
{
	//スケーリング行列を宣言
	Matrix4x4 matScale;
	matScale.IdentityMatrix();
...
Matrix4x4 DrawOBJ::matRotateGeneration(Vector3 rotate)
{
	//X軸回転行列を宣言
...
Matrix4x4 DrawOBJ::matMoveGeneration(Vector3 translation)
{
	//移動するための行列を用意
	Matrix4x4 matMove;
...
Matrix4x4 DrawOBJ::matViewGeneration(Vector3 eye, Vector3 target, Vector3 up)
{
	...
	Vector3 D0 = Vector3Dat(R0, NegEyePosition);
	Vector3 D1 = Vector3Dat(R1, NegEyePosition);
	Vector3 D2 = Vector3Dat(R2, NegEyePosition);
...
Vector3 DrawOBJ::Vector3Dat(Vector3 A, Vector3 B)
{
